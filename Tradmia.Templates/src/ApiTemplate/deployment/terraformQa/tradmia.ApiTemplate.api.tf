...
resource "azurerm_linux_web_app" "ApiTemplate-api" {
  ...
  site_config {
    ...
    ip_restriction {
        name = "junoaggservice-api-qa-ip1"
        priority = 3
        action = "Allow"
        ip_address = "51.137.4.254/32"
    }

     ip_restriction {
        name = "junoaggservice-api-qa-ip2"
        priority = 3
        action = "Allow"
        ip_address = "51.138.63.47/32"
    }

     ip_restriction {
        name = "junoaggservice-api-qa-ip3"
        priority = 3
        action = "Allow"
        ip_address = "51.138.63.50/32"
    }

     ip_restriction {
        name = "junoaggservice-api-qa-ip4"
        priority = 3
        action = "Allow"
        ip_address = "51.138.63.62/32"
    }

     ip_restriction {
        name = "junoaggservice-api-qa-ip5"
        priority = 3
        action = "Allow"
        ip_address = "51.138.63.72/32"
    }

     ip_restriction {
        name = "junoaggservice-api-qa-ip6"
        priority = 3
        action = "Allow"
        ip_address = "51.138.63.88/32"
    }

    ip_restriction {
        name = "junoaggservice-api-qa-ip7"
        priority = 3
        action = "Allow"
        ip_address = "20.50.2.43/32"
    }
  }
